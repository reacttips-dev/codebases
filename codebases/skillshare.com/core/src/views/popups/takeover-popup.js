...
const TakeoverPopupView = BaseView.extend({

  id: 'takeover-popup-view',

  // flag to auto-render after initialize. see base/view.js
  autoRender: true,

  // container element. see base/view.js
  container: '#click-off-overlay',
  ...
  initialize: function(options = {}) {
    ...
    if (this.disposeOnClose) {
      this.listenTo(PageOverlay, 'overlayDidCloseEVENT', () => this.trigger('onPopupDidCloseEvent'));
    }
  ...
  createWithContentFromEndpoint: function(endpoint, endpointData) {
    ...
    if (this.shouldShowLoaderIcon !== false) {
      this.addLoaderIcon();
    }
  ...
  createWithView: function(view) {
    ...
    const _this = this;
    this.subview('content').on('onContentReady', () => {
      _this.render();
      _this.openPopup();
    });
  },
  ...
  openPopup: function() {
    PageOverlay.open({closeOnlyWithTheXButton: true});
    this.$el.show();
  ...
  closePopup: function(forceClose) {
    if (forceClose === true || this.preventClose !== true) {
      ...
      if (PopupStack.getStack().length > 1) {
        ...
        PageOverlay.close();
        SS.events.trigger('takeover:video:play');
      }
    }
  },
...
export default TakeoverPopupView;
