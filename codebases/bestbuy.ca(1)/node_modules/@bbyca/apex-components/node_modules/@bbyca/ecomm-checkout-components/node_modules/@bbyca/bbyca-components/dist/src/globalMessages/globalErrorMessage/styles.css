

@media (min-width: 768px) {
    .globalMessageContainer_49h-0 {
        font-size: 14px;
        line-height: 20px;
    }

    .globalMessageContainer_49h-0 p {
        font-size: 14px;
    }
}

.globalMessageContainer_49h-0 {
    font-size: 14px;
    -webkit-box-align: center;
    -webkit-align-items: center;
            align-items: center;
    position: relative;
    line-height: 20px;
    padding: 0 0 0 40px;
    border-radius: 4px;
    border: 2px solid;
}

.globalMessageContainer_49h-0 p,
.globalMessageContainer_49h-0 .contentContainer_3trC4 p {
    font-size: 14px;
    margin: 0 0 8px;
    padding: 0;
}

.globalMessageContainer_49h-0 p:last-child,
.globalMessageContainer_49h-0 .contentContainer_3trC4 p:last-child {
    margin-bottom: 0px;
}

.globalMessageContainer_49h-0 p.message_1bXu0 {
    font-weight: 600;
    margin-bottom: 8px;
}

.contentContainer_3trC4 {
    background-color: #fff;
    padding: 16px;
}

.contentContainer_3trC4 a {
    font-size: 14px;
    display: inline-block;
    line-height: 20px;
    color: #0046be;
    font-weight: 600;
    margin-top: 8px;
    text-decoration: none;
}

.contentContainer_3trC4 a svg {
    width: 1em;
    height: 1em;
}

.iconContainer_2v4Yd {
    position: absolute;
    left: 8px;
    top: 50%;
    margin: 0 auto;
    -webkit-transform: translateY(-50%);
            transform: translateY(-50%);
    width: 24px;
    height: 24px;
}

.contentContainer_3trC4 a {
    font-size: 14px;
    display: inline-block;
    line-height: 20px;
    color: #0046be;
    font-weight: 600;
    margin-top: 8px;
    text-decoration: none;
}

.contentContainer_3trC4 a svg {
    width: 1em;
    height: 1em;
}

.globalMessageContainer_49h-0 {
    background-color: #bb0628;
    border-color: #bb0628;
    margin-bottom: 16px;
}

.globalMessageContainer_49h-0 :before {
        display: none;
    }

.iconContainer_2v4Yd {
    background: url(data:image/svg+xml;base64,) no-repeat;
}
