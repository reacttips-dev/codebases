

@media (min-width: 768px) {
    .globalMessageContainer__mHRu {
        font-size: 14px;
        line-height: 20px;
    }

    .globalMessageContainer__mHRu p {
        font-size: 14px;
    }
}

.globalMessageContainer__mHRu {
    font-size: 14px;
    -webkit-box-align: center;
    -webkit-align-items: center;
            align-items: center;
    position: relative;
    line-height: 20px;
    padding: 0 0 0 40px;
    border-radius: 4px;
    border: 2px solid;
}

.globalMessageContainer__mHRu p,
.globalMessageContainer__mHRu .contentContainer_pJk1y p {
    font-size: 14px;
    margin: 0 0 8px;
    padding: 0;
}

.globalMessageContainer__mHRu p:last-child,
.globalMessageContainer__mHRu .contentContainer_pJk1y p:last-child {
    margin-bottom: 0px;
}

.globalMessageContainer__mHRu p.message_2IuYK {
    font-weight: 600;
    margin-bottom: 8px;
}

.contentContainer_pJk1y {
    background-color: #fff;
    padding: 16px;
}

.contentContainer_pJk1y a {
    font-size: 14px;
    display: inline-block;
    line-height: 20px;
    color: #0046be;
    font-weight: 600;
    margin-top: 8px;
    text-decoration: none;
}

.contentContainer_pJk1y a svg {
    width: 1em;
    height: 1em;
}

.iconContainer_36zQj {
    position: absolute;
    left: 8px;
    top: 50%;
    margin: 0 auto;
    -webkit-transform: translateY(-50%);
            transform: translateY(-50%);
    width: 24px;
    height: 24px;
}

.contentContainer_pJk1y a {
    font-size: 14px;
    display: inline-block;
    line-height: 20px;
    color: #0046be;
    font-weight: 600;
    margin-top: 8px;
    text-decoration: none;
}

.contentContainer_pJk1y a svg {
    width: 1em;
    height: 1em;
}

.globalMessageContainer__mHRu {
    background-color: #bb0628;
    border-color: #bb0628;
    margin-bottom: 16px;
}

.globalMessageContainer__mHRu :before {
        display: none;
    }

.iconContainer_36zQj {
    background: url(data:image/svg+xml;base64,) no-repeat;
}
