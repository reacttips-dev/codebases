

@media (min-width: 768px) {
    .globalMessageContainer_2EdiD {
        font-size: 14px;
        line-height: 20px;
    }

    .globalMessageContainer_2EdiD p {
        font-size: 14px;
    }
}

.globalMessageContainer_2EdiD {
    font-size: 14px;
    -webkit-box-align: center;
    -webkit-align-items: center;
            align-items: center;
    position: relative;
    line-height: 20px;
    padding: 0 0 0 40px;
    border-radius: 4px;
    border: 2px solid;
}

.globalMessageContainer_2EdiD p,
.globalMessageContainer_2EdiD .contentContainer_2tzCc p {
    font-size: 14px;
    margin: 0 0 8px;
    padding: 0;
}

.globalMessageContainer_2EdiD p:last-child,
.globalMessageContainer_2EdiD .contentContainer_2tzCc p:last-child {
    margin-bottom: 0px;
}

.globalMessageContainer_2EdiD p.message_2A8MJ {
    font-weight: 600;
    margin-bottom: 8px;
}

.contentContainer_2tzCc {
    background-color: #fff;
    padding: 16px;
}

.contentContainer_2tzCc a {
    font-size: 14px;
    display: inline-block;
    line-height: 20px;
    color: #0046be;
    font-weight: 600;
    margin-top: 8px;
    text-decoration: none;
}

.contentContainer_2tzCc a svg {
    width: 1em;
    height: 1em;
}

.iconContainer_O4l0F {
    position: absolute;
    left: 8px;
    top: 50%;
    margin: 0 auto;
    -webkit-transform: translateY(-50%);
            transform: translateY(-50%);
    width: 24px;
    height: 24px;
}

.contentContainer_2tzCc a {
    font-size: 14px;
    display: inline-block;
    line-height: 20px;
    color: #0046be;
    font-weight: 600;
    margin-top: 8px;
    text-decoration: none;
}

.contentContainer_2tzCc a svg {
    width: 1em;
    height: 1em;
}

.globalMessageContainer_2EdiD {
    background-color: #308800;
    border-color: #308800;
    margin-bottom: 16px;
}

.globalMessageContainer_2EdiD :before {
        display: none;
    }

.iconContainer_O4l0F {
    background: url(data:image/svg+xml;base64,) no-repeat;
}
