...
const bind = (callback) => clickAwayEvents.forEach((event) => document.addEventListener(event, callback, { passive: false }));
        ...
        this.handleClickAway = (event) => {
            ...
            if (document.documentElement.contains(event.target)) {
                if (!isDescendant(el, event.target)) {
                    if (event.target.classList.contains("x-global-overlay")) {
