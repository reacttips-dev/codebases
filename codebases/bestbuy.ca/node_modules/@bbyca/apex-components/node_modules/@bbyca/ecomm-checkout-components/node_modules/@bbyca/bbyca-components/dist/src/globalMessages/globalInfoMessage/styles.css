

@media (min-width: 768px) {
    .globalMessageContainer_3Szr_ {
        font-size: 14px;
        line-height: 20px;
    }

    .globalMessageContainer_3Szr_ p {
        font-size: 14px;
    }
}

.globalMessageContainer_3Szr_ {
    font-size: 14px;
    -webkit-box-align: center;
    -webkit-align-items: center;
            align-items: center;
    position: relative;
    line-height: 20px;
    padding: 0 0 0 40px;
    border-radius: 4px;
    border: 2px solid;
}

.globalMessageContainer_3Szr_ p,
.globalMessageContainer_3Szr_ .contentContainer_2cVuq p {
    font-size: 14px;
    margin: 0 0 8px;
    padding: 0;
}

.globalMessageContainer_3Szr_ p:last-child,
.globalMessageContainer_3Szr_ .contentContainer_2cVuq p:last-child {
    margin-bottom: 0px;
}

.globalMessageContainer_3Szr_ p.message_iXhw6 {
    font-weight: 600;
    margin-bottom: 8px;
}

.contentContainer_2cVuq {
    background-color: #fff;
    padding: 16px;
}

.contentContainer_2cVuq a {
    font-size: 14px;
    display: inline-block;
    line-height: 20px;
    color: #0046be;
    font-weight: 600;
    margin-top: 8px;
    text-decoration: none;
}

.contentContainer_2cVuq a svg {
    width: 1em;
    height: 1em;
}

.iconContainer_3OTFy {
    position: absolute;
    left: 8px;
    top: 50%;
    margin: 0 auto;
    -webkit-transform: translateY(-50%);
            transform: translateY(-50%);
    width: 24px;
    height: 24px;
}

.contentContainer_2cVuq a {
    font-size: 14px;
    display: inline-block;
    line-height: 20px;
    color: #0046be;
    font-weight: 600;
    margin-top: 8px;
    text-decoration: none;
}

.contentContainer_2cVuq a svg {
    width: 1em;
    height: 1em;
}

.globalMessageContainer_3Szr_ {
    background-color: #0046be;
    border-color: #0046be;
    margin-bottom: 16px;
}

.globalMessageContainer_3Szr_ :before {
        display: none;
    }

.iconContainer_3OTFy {
    background: url(data:image/svg+xml;base64,) no-repeat;
}

.chevron_3R-O5 {
    height: 16px;
    position: relative;
    line-height: 1;
    vertical-align: middle;
    fill: #0046be;
}
