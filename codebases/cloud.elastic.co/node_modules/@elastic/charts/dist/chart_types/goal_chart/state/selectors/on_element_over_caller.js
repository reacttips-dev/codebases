...
function createOnElementOverCaller() {
    ...
    return function (state) {
        if (selector === null && state.chartType === __1.ChartType.Goal) {
            selector = create_selector_1.createCustomCachedSelector([goal_spec_1.getSpecOrNull, picked_shapes_1.getPickedShapesLayerValues, get_settings_specs_1.getSettingsSpecSelector], event_handler_selectors_1.getOnElementOverSelector(prev));
        }
        if (selector) {
            selector(state);
        }
