...
function createOnElementClickCaller() {
    ...
    return function (state) {
        if (selector === null && state.chartType === __1.ChartType.Goal) {
            selector = create_selector_1.createCustomCachedSelector([goal_spec_1.getSpecOrNull, get_last_click_1.getLastClickSelector, get_settings_specs_1.getSettingsSpecSelector, picked_shapes_1.getPickedShapesLayerValues], event_handler_selectors_1.getOnElementClickSelector(prev));
        }
        if (selector) {
            selector(state);
        }
