...
function createOnElementOutCaller() {
    ...
    return function (state) {
        if (selector === null && state.chartType === __1.ChartType.Partition) {
            selector = create_selector_1.createCustomCachedSelector([partition_spec_1.getPartitionSpec, picked_shapes_1.getPickedShapesLayerValues, get_settings_specs_1.getSettingsSpecSelector], event_handler_selectors_1.getOnElementOutSelector(prev));
        }
        if (selector) {
            selector(state);
        }
