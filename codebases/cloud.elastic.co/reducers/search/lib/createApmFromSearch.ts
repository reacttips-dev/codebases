...
export default function createApmFromSearch({
  regionId,
  id,
  source,
  highlight,
  ...
  const currentVersion = get(plan, [`current`, `apm`, `version`])
  const pendingVersion = get(plan, [`pending`, `apm`, `version`])
  ...
  const isHidden = get(data, [`data`, `hidden`], false)
  const hiddenTimestamp = get(data, [`data`, `hidden_timestamp`])
