...
export default function createAppSearchFromSearch({
  regionId,
  id,
  source,
  highlight,
  ...
  const currentVersion = get(plan, [`current`, `appsearch`, `version`])
  const pendingVersion = get(plan, [`pending`, `appsearch`, `version`])
