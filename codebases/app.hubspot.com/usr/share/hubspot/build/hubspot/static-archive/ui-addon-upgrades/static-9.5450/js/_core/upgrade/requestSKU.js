...
import { sendRequestSKUNotification } from 'ui-addon-upgrades/_core/common/api/requestNotificationsApi';
...
export var requestSKU = wrapWithErrorMonitoring('requestSKU', function (_ref) {
  var apiName = _ref.apiName;
  return sendRequestSKUNotification(apiName).then(function () {
    // ********** PUBLIC EVENT **********
    ...
    tracker.track('requestUpgradeInteraction', {
      action: 'request sku notification succeeded',
      apiName: apiName
    });
  }).catch(function (e) {
    // ********** PUBLIC EVENT **********
    ...
    // Read more about Public Events on the wiki: https://wiki.hubspotcentral.net/display/PM/Public+Events+-+Amplitude+events+ready+for+HubSpot+team+use+and+automation
    tracker.track('requestUpgradeInteraction', {
      action: 'request sku notification failed',
      apiName: apiName
    });
    Raven.captureMessage('Request sku notification failed', {
      extra: {
        error: e
      }
    });
