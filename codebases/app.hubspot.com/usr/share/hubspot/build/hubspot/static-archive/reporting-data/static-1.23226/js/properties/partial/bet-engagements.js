...
/* Cross-object additional bet properties */

export var getBetEngagementsPropertyGroups = function getBetEngagementsPropertyGroups() {
  return isBetPortal() ? fromJS([{
    ...
    hubspotDefined: true,
    properties: [{
      name: 'hs_unknown_visitor_conversation',
      label: translate('unknownVisitorConversation'),
      type: 'bool'
    }, {
...
export var withBetGroups = function withBetGroups(groups) {
  if (!isBetPortal()) {
    return groups;
  }

  var engagementGroupIndex = groups.findIndex(function (group) {
    return group.get('name') === 'engagement';
  });

  if (engagementGroupIndex === -1) {
    return groups.concat(getBetEngagementsPropertyGroups());
  }

  return groups.updateIn([engagementGroupIndex, 'properties'], function (properties) {
    var engagementGroupPropertyMap = properties.toMap().mapKeys(function (k, value) {
      return value.get('name');
    });
    var betGroupPropertyMap = getBetEngagementsPropertyGroups().first().get('properties').toMap().mapKeys(function (_, value) {
      return value.get('name');
    });
    return engagementGroupPropertyMap.mergeDeep(betGroupPropertyMap).toList();
  });
};