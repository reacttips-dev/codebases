  ...
  return get(URLS.connectedAccounts, {
    ...
    if (!hasOwnerAccount) {
      ...
      return getPortalEmailSignature().then(function (sig) {
        ...
        logError({
          error: new Error('Failed to fetch the email signature for the owner connected account.'),
          extraData: {
            error: error
          },
          tags: {
...
export function fetchAndHydrateConnectedAccounts() {
  var userEmail = User.get().get('email').toLowerCase();
  return getConnectedAccounts(userEmail).catch(function (error) {
    logError({
      error: new Error('Failed to fetch the connected accounts.'),
      extraData: {
        error: error
      },
      tags: {
