...
var transformOriginMixin = css(["", ";", ";"], ['top', 'bottom', 'left', 'right'].map(function (side) {
  return ".private-popover-attached-" + side + " & { transform-origin: " + side + " }";
}), ['top', 'bottom'].map(function (ySide) {
  return ['left', 'right'].map(function (xSide) {
    return ".private-popover-attached-" + ySide + ".private-popover-attached-" + xSide + " & { transform-origin: " + ySide + " " + xSide + " }";
  });
}));
...
var UIPopoverContent = /*#__PURE__*/forwardRef(function (props, ref) {
  var arrowColor = props.arrowColor,
      ...
      width = props.width,
      rest = _objectWithoutProperties(props, ["arrowColor", "arrowSize", "className", "content", "Content", "renderContent", "onCloseComplete", "onCloseStart", "onKeyDown", "onOpenComplete", "onOpenStart", "open", "style", "use", "animate", "showCloseButton", "onOpenChange", "width"]);
