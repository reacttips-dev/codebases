...
import { SvgConfused, SvgaConfusedFilled } from '@coursera/coursera-ui/svg';
...
const DislikeV2 = (props: Props) => {
  const { selected, tooltipPlacement } = props;

  const tooltip = <Tooltip>{_t('Confusing')}</Tooltip>;

  return (
    <div className="rc-Dislike">
      <LikeContent {...props} placeholder={_t('This item was confusing because...')} isConfusingFeedback>
        <OverlayTrigger placement={tooltipPlacement} overlay={tooltip} delayShow={300}>
          <button type="button" className="c-button-icon" aria-pressed={selected} aria-label={_t('Confusing')}>
            {selected ? <SvgaConfusedFilled size={20} /> : <SvgConfused size={20} color={color.icon} />}
          </button>
        </OverlayTrigger>
      </LikeContent>
    </div>
