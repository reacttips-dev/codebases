...
import ItemFeedbackContent from 'bundles/content-feedback/components/cds/item-feedback/ItemFeedbackContent';
...
class ItemFeedback extends React.Component<PropsToComponent> {
  ...
  render() {
    const { computedItem, itemFeedbackType, courseId, subItemId, tooltipPlacement, className } = this.props;
    const { id: itemId } = computedItem;

    const isInvalidItemType = values(ItemTypes).indexOf(itemFeedbackType) === -1;
    ...
    return (
      <div className={`rc-ItemFeedback ${className}`}>
        <ItemFeedbackContent
          key={itemId}
...
class TrackedItemFeedback extends React.Component<PropsToComponent> {
  ...
  render() {
    const { courseId, computedItem, itemFeedbackType, subItemId, tooltipPlacement, className } = this.props;
