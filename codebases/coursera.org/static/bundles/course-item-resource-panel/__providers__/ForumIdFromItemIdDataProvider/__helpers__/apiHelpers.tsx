export function objToParams(obj: Record<string, string | number>): string {
...
export function linkedToArguments<LinkedItemsType>(linkedItems: LinkedItemsType) {
  ...
  for (let i = 0; i < keys.length; i += 1) {
    ...
    if (fields && Array.isArray(fields)) {
      names.push(api + '(' + subparams(fields) + ')');
    }
  }
...
export function getURI<Args extends BaseInterface>(args: Args) {
  ...
  let path = (args && args.path && (typeof args?.path === 'string' ? args.path : getPath(args.params))) || '';
