/**
 * NOTE: This is generated file any changes to it can be overwritten at some point
 */
import React from 'react';

import { createSvgIcon } from '@coursera/cds-core';

export default createSvgIcon(
  'ClockFilledIcon',
  {
    ltr: {
      small: ({ id }) => (
        <>
          <path d="M8 15A7 7 0 108 1a7 7 0 000 14z" fill="#F2D049" />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M8 1.5a6.5 6.5 0 100 13 6.5 6.5 0 000-13zM.5 8a7.5 7.5 0 1115 0 7.5 7.5 0 01-15 0z"
            fill="#7C6919"
          />
          <path d="M8 3v5l3 3" fill="#F2D049" />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M7.5 3h1v4.793l2.854 2.853-.708.708L7.5 8.207V3z"
            fill="currentColor"
          />
        </>
      ),
      medium: ({ id }) => (
        <>
          <path d="M10 19a9 9 0 100-18 9 9 0 000 18z" fill="#F2D049" />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M10 1.5a8.5 8.5 0 100 17 8.5 8.5 0 000-17zM.5 10a9.5 9.5 0 1119 0 9.5 9.5 0 01-19 0z"
            fill="#7C6919"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M9.5 4h1v5.793l3.854 3.853-.708.708L9.5 10.207V4z"
            fill="currentColor"
          />
        </>
      ),
      large: ({ id }) => (
        <>
          <path
            d="M12 23c6.075 0 11-4.925 11-11S18.075 1 12 1 1 5.925 1 12s4.925 11 11 11z"
            fill="#F2D049"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M12 1.5C6.201 1.5 1.5 6.201 1.5 12S6.201 22.5 12 22.5 22.5 17.799 22.5 12 17.799 1.5 12 1.5zM.5 12C.5 5.649 5.649.5 12 .5S23.5 5.649 23.5 12 18.351 23.5 12 23.5.5 18.351.5 12z"
            fill="#7C6919"
          />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M11.5 4h1v7.793l4.854 4.853-.708.708-5.146-5.147V4z"
            fill="currentColor"
          />
        </>
      ),
    },
  },
  false
);
