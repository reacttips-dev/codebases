...
const activitiesConstants_1 = require("../constants/activitiesConstants");
 ...
 */
const generateActivitiesQuery = ({ fields, shouldQueryList, shouldQueryExportList, shouldQueryChartData, reportType, interval, groupByFilter, segmentByFilter, measureByFilter, stats, filterByFilter, chartType, timeZone, }) => {
    const intervalToUse = lodash_1.isString(interval) ? interval : '';
    const queryBuilder = new flatQueryBuilder_1.default(types_1.DataType.ACTIVITIES);
    const reportTypeToUse = reportType === types_1.ReportType.ACTIVITIES_STATS ? 'stats' : reportType;
    const listFieldsWithLabelFields = fields
        ? private_1.getFieldsWithLabelFields(Object.keys(fields), activitiesConstants_1.FIELDS_WITH_LABELS)
        : [];
    const statsFieldsWithLabelFields = stats ? private_1.getFieldsWithLabelFields(Object.keys(stats), activitiesConstants_1.FIELDS_WITH_LABELS) : [];
    const query = queryBuilder
        .setShouldQueryList(shouldQueryList)
        .setShouldQueryExportList(shouldQueryExportList)
        .setShouldQueryChart(shouldQueryChartData)
        .setFilters(filterByFilter)
        .setMeasureBy(measureByFilter)
        .setTimeZone(timeZone);
    if (!activitiesConstants_1.CHART_TYPES_WITHOUT_GROUP_BY.includes(chartType)) {
        query.setGroupBy(groupByFilter, intervalToUse);
    }
    if (!activitiesConstants_1.CHART_TYPES_WITHOUT_SEGMENT_BY.includes(chartType)) {
...
exports.default = generateActivitiesQuery;
