...
import { onLikeNotificationClicked } from 'owa-header-app-notifications';
...
export default orchestrator(onLikeNotificationClicked, async message => {
    ...
    if (isConversationView(getSelectedTableView())) {
        ...
        if (!cachedConversation) {
            ...
            await lazyLoadConversation.importAndExecute(conversationId, 'LoadLikedConversation');
        }
        ...
        if (cachedItem?.NormalizedBody) {
            ...
            await lazyLoadItem.importAndExecute(itemId, 'LoadLikedItem');
            openItemInNewTab(itemId, notification.subject);
        }
    }
});
...
export type { LikeNotificationData } from 'owa-header-app-notifications';
