...
import { USER_OCPS_NPS_SURVEY_ENABLED } from '../utils/constants';
import fetchPoliciesAndSetStore from './fetchPoliciesAndSetStore';
import isConsumer from 'owa-session-store/lib/utils/isConsumer';
import { isFeatureEnabled } from 'owa-feature-flags';

export default async function isNpsSurveyPolicyEnabled(): Promise<boolean> {
    ...
    if (localStorageExists(window)) {
        ...
        if (itemExists(window, USER_OCPS_NPS_SURVEY_ENABLED)) {
            return getItem(window, USER_OCPS_NPS_SURVEY_ENABLED) == '1';
        }
    }
