...
const lazyModule = new LazyBootModule(() => {
    ...
    if (
        ...
        OWA_BUILD_CONSTANTS.ENTRIES.mail
    ) {
        return import(/* webpackChunkName: "MailDeepBoot" */ './lazyIndex');
    } else {
        return Promise.reject(new Error('Entrypoint is not being built'));
    }
});

export const lazyGetMailDeepLinkBootstrapOptions = new LazyAction(
    lazyModule,
    m => m.getMailDeepLinkBootstrapOptions
);
