        ...
        yAxisLabelsConfig,
        {
            ...
            xAxis: {
                ...
                labels: {
                    ...
                    formatter() {
                        const { value } = this;
                        if (isWithinComparisonMode) {
                            return this.isLast
                                ? i18nFilter()(`audience.loyalty.comparison.column.xAxis.last`)
                                : i18nFilter()(
                                      `audience.loyalty.comparison.column.xAxis.${value}`,
                                      { bucket: value },
                                  );
                        }
                        return i18nFilter()(`audience.loyalty.column.xAxis.${value}`, {
                            bucket: value,
                        });
                    },
                },
            ...
            plotOptions: {
                column: {
                    ...
                    dataLabels: {
                        ...
                        overflow: "none",
                        formatter() {
                            return yAxisFormatter({ value: this.y });
                        },
                    },
                    states: {
                        hover: {
...
export function singleDomainTooltipFormatter(filter, outside) {
    const lines = [];
    if (outside) {
        lines.push(`<div class="sharedTooltip">`);
        lines.push(`<div class="highcharts-tooltip">`);
    }
    lines.push(`<div class="date">${i18nFilter()("audience.loyalty.graph.tooltip.title")}</div>`);
    _.forEach(this.points, (point) => {
        ...
        const seriesName =
            point?.point?.category === LOYALTY_CATEGORIES[5]
                ? `${i18nFilter()("audience.loyalty.graph.tooltip.text.5+", {
                      bucket: point?.point?.category,
                  })}`
                : `${i18nFilter()("audience.loyalty.graph.tooltip.text", {
                      bucket: point?.point?.category,
                  })}`;
        const seriesSubtitle = point.series.userOptions.seriesSubtitle;
        lines.push(`<div style="display: flex;align-items: baseline; min-width: 200px; max-width: 280px; padding: 1px 6px">
