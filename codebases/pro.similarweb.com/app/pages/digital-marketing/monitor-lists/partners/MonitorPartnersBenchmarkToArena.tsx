...
const MonitorPartnersBenchmarkToArena = (props) => {
    const { country, partnerListId, webSource, duration, sites } = props.params;
    ...
    useEffect(() => {
        async function getArenasFromWorkspaces() {
            setLoadingArenas(true);
            let workspaces;
            try {
                workspaces = await marketingWorkspaceApiService.getMarketingWorkspaces();
            } catch (e) {
                swLog.error(`Error fetching arenas - MonitorPartnersBenchmarkToArena -- ${e}`);
                return null;
            }
        ...
        if (!sites) {
            getArenasFromWorkspaces();
        }
    }, [country, partnerListId, webSource, duration, sites]);
    ...
    const getAvailableArenas = async (allArenas) => {
        ...
        const endpoint = `api/MonitorPartnerList/${partnerListId}/benchmark/overview`;
        ...
        allArenas.forEach(async (arena: IArena, index) => {
            ...
            try {
                const { visits } = await fetchService.get<{ visits: number; change: number }>(
                    endpoint,
                    finalParams,
    ...
    const onArenaClick = (arena) => () => {
        TrackWithGuidService.trackWithGuid("monitor.website.list.page.arena.selected", "close", {
...
const connected = connect(mapStateToProps, null)(MonitorPartnersBenchmarkToArena);

export default SWReactRootComponent(connected, "MonitorPartnersBenchmarkToArena");
