...
export class IndustryAnalysisTopKeywordsOrganic extends IndustryAnalysisTopKeywordsBase {
    ...
    static getAllConfigs(params) {
        const config = _.cloneDeep(
            IndustryAnalysisTopKeywordsAll.getAllConfigs(params, ETopKeywordsTable.organic),
        );
        config.metricConfig.state = "keywordAnalysis-organic";
        const { apiParams } = config.widgetConfig.properties;
        const newApiParams = addFilter(apiParams)("OP;==;0");
    ...
    getWidgetModel() {
        ...
        model.metric = "TopKeywordsOrganic";
        return model;
    }
