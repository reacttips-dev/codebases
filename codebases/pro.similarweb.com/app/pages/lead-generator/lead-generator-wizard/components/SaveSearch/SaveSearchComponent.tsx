...
import {
    ...
    StyledSaveSearchLastUpdateWrapper,
...
class SaveSearch extends PureComponent<ISaveSearchProps, ISaveSearchState> {
    ...
    renderAutoReRunSwitch() {
        ...
        const isActiveSwitch = activeSavedSearchListData?.queryDefinition?.auto_rerun_activated;

        let disabledReasonText = translate(SAVE_SEARCH_RE_RUN_TOOLTIP_DISABLED_LIMIT, {
            number: searchesReRunLimitCount,
        });

        if (searchesReRunLimitCount === 0) {
            disabledReasonText = translate(SAVE_SEARCH_RE_RUN_TOOLTIP_DISABLED_TRIAL);
        }
    ...
    render() {
        ...
        const lastUpdateDate = dayjs
            .utc(activeSavedSearchListData?.lastRun?.endTime)
            .utc()
            .format("MM/DD/YYYY");
        ...
        return (
            <>
                {this.state.isShowSaveBtn ? (
                    ...
                    <StyledSaveSearchActionsWrapper>
                        <StyledSaveSearchLastUpdateWrapper>
                            {i18nFilter()(SAVE_SEARCH_LAST_UPDATE_TEXT)}
                            {": "}
                            {lastUpdateDate}
                        </StyledSaveSearchLastUpdateWrapper>
                        {this.renderAutoReRunSwitch()}
                        <StyledSaveSearchSettingIconWrapper>
                            <IconButton
