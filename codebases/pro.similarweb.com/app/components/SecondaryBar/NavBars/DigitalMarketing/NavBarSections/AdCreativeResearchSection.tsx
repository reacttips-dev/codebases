...
import { navObj } from "pages/digital-marketing/config/navigation/adCreativeResearchNavObj";
...
const AD_CREATIVE_RESEARCH_STATE_ID = "adcreativeresearch_home";

interface IAdCreativeResearchSectionProps {
    currentPage: string;
...
export const AdCreativeResearchSection: FC<IAdCreativeResearchSectionProps> = ({
    currentPage,
    navigator,
    rootScope,
    onSectionClick,
    ...
    useEffect(() => {
        currentSection && currentSection !== AD_CREATIVE_RESEARCH_STATE_ID && setSectionOpen(false);
    }, [currentSection]);
    ...
    return sectionItems?.length > 0 ? (
        <NavBarHomepageItem
            id={AD_CREATIVE_RESEARCH_STATE_ID}
            text={i18n("aquisitionintelligence.navbar.adcreativeresearch.header")}
